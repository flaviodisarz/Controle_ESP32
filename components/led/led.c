...
void led_init(const led_config_t* cfg)
{
    ...
    s_cfg.pulse_period_ms  = 300000; // ✅ 5 min certinho
    ...
    s_q = xQueueCreate(16, sizeof(led_cmd_t)); // ✅ maior pra aguentar sequências (2/3 piscadas)
    if (!s_q) {
        ESP_LOGE(TAG, "queue create failed");
        return;
    }

    // driver WS2812 via RMT
...
{
    // Mantido (pra debug futuro), mas no state.c a gente não chama mais.
    (void)id;
}
